...
local SBC = SBC or {} --Localise the global table for speed.
SBC.Utl = {} --Make a Utility Table.
local Utl = SBC.Utl --Makes it easier to read the code.
...
local DTable = SBC.Utl.DebugTable --Localise the debug storage.
local HTable = SBC.Utl.Hooks --Localise the hook table for speed.
...
function Utl:Debug(Source,String,Type)
	SBC.Debug(String,2,"["..Type.."]"..Source)--Redirect this to use the debug.lua debug functions.
...
function Utl:MakeHook(Name) --Make the hookhook storage.
	if(not HTable[Name])then
		...
		hook.Add(Name,"SBCHookMake",Func)
	else
